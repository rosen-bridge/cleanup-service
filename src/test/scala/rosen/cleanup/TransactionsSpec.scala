...
class TransactionsSpec extends TestSuite {
  ...
  property("generateFrauds constructs valid transaction") {
    ...
    val fraudBoxTokens = Seq(new ErgoToken(Configs.tokens.RWT, 1))
   ...
   */
  property("slashFraud constructs valid transaction, removes WID if it has 0 RWT") {
    // initialize test data
    val repoBox = TestBoxes.mockRepoBox(5, (blockchainHeight - 10).toInt)
    val fraudBox = TestBoxes.mockFraudBox(repoBox.getWIDs.head, (blockchainHeight - 5).toInt)
    ...
    val repoBoxTokens = Seq(
      ...
      new ErgoToken(Configs.tokens.RWT, 101),
    ...
    newWIDs should equal(repoBox.getWIDs.slice(1, 5))
    val newRWTs = outputRepo.getRWTs
    newRWTs should equal(repoBox.getRWTs.slice(1, 5))
   ...
   *    Third output should be cleaner box
   */
  property("slashFraud constructs valid transaction, reduces RWT by 1") {
    // initialize test data
    val repoBox = TestBoxes.mockRepoBox(5, (blockchainHeight - 10).toInt)
    ...
    val newRWTs = outputRepo.getRWTs
    newRWTs(1) should equal(repoBox.getRWTs(1) - 1)
