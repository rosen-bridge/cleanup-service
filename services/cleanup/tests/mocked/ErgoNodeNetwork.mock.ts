...
const ergoNodeNetworkMocks = vi.hoisted(() => {
  ...
  const getErgoStateContextMock = vi.fn().mockImplementation(async () => {
    const headersJson = last10BlockHeaders.map((h: any) =>
...
export const isBoxUnspentAndValidMock = ergoNodeNetworkMocks.isBoxUnspentAndValidMock;
export const getErgoStateContextMock = ergoNodeNetworkMocks.getErgoStateContextMock;
export const ergoNodeNetworkCtorMock = ergoNodeNetworkMocks.ergoNodeNetworkCtorMock;
